...
import { Folder, FileCode, FileText, Terminal, Code2 } from "lucide-react";
...
export default function IDEDashboard() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#1e1e1e] text-[#d4d4d4] font-mono">
      {/* Activity Bar */}
      <div className="w-12 bg-[#333333] flex flex-col items-center py-4 border-r border-[#2a2a2a] shrink-0">
        <Code2 className="text-[#569cd6] w-6 h-6 mb-6" />
      </div>

      {/* Sidebar Explorer */}
      <div className="w-64 bg-[#252526] border-r border-[#2a2a2a] flex flex-col select-none shrink-0">
        <div className="px-4 py-2 text-xs font-bold text-[#858585] tracking-wider uppercase">
          Explorer
        </div>
        ...
        <div className="mt-2 text-sm">
          <div
            ...
            className={`flex items-center gap-2 px-4 py-1.5 cursor-pointer hover:bg-[#2a2d2e] ${
          ...
          {portfolioData.projects.map((proj) => (
            <div
              ...
              className={`flex items-center gap-2 pl-8 pr-4 py-1.5 cursor-pointer hover:bg-[#2a2d2e] ${
      ...
      <div className="flex-1 flex flex-col bg-[#1e1e1e] min-w-0">
        {/* Workspace Tabs */}
        <div className="flex bg-[#252526] border-b border-[#2a2a2a] overflow-x-auto">
          <button
            onClick={() => setActiveTab("resume")}
            className={`flex items-center gap-2 px-4 py-2 text-xs border-r border-[#2a2a2a] shrink-0 ${
              activeTab === "resume"
        ...
        <div className="flex-1 p-6 overflow-y-auto text-sm leading-relaxed">
          {activeTab === "resume" ? (
            <div className="space-y-6 max-w-4xl">
              {/* Header */}
              <div>
                <h1 className="text-2xl font-bold text-[#569cd6]"># {portfolioData.resume.name}</h1>
                <p className="text-[#4ec9b0] font-semibold">{portfolioData.resume.role}</p>
                <div className="text-xs text-[#808080] mt-2 flex flex-wrap gap-x-4 gap-y-1">
              ...
              {/* Summary */}
              <div>
                <h2 className="text-lg font-semibold text-[#4ec9b0]">## Professional Summary</h2>
                <p className="text-[#ce9178] mt-1">"{portfolioData.resume.summary}"</p>
              </div>

              {/* Technical Skills */}
              <div>
                <h2 className="text-lg font-semibold text-[#4ec9b0]">## Technical Skills</h2>
              ...
              {/* Experience */}
              <div>
                <h2 className="text-lg font-semibold text-[#4ec9b0]">## QA & Professional Experience</h2>
                <div className="space-y-4 mt-3">
                  {portfolioData.resume.experience.map((exp, idx) => (
                    <div key={idx} className="border-l-2 border-[#007acc] pl-4 py-1">
              ...
              {portfolioData.resume.education && (
                <div>
                  <h2 className="text-lg font-semibold text-[#4ec9b0]">## Education & Training</h2>
            ...
            <div className="space-y-4 max-w-3xl">
              <div className="text-[#6a9955]">// Module Source File</div>
              <h1 className="text-xl font-bold text-[#dcdcaa]">{activeProject.name}</h1>
              <p className="text-[#ce9178]">/* {activeProject.description} */</p>

              {activeProject.id === "srs-test-generator" ? (
                <div className="bg-[#181818] border border-[#2a2a2a] rounded p-4 text-xs font-mono space-y-4">
                  <div>
  ...
  <div className="mt-1 text-[#dcdcaa] overflow-x-auto">
    ...
    &#125;);
              ...
              <div className="pt-4">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 bg-[#0e639c] hover:bg-[#1177bb] text-white px-4 py-2 rounded text-xs transition-colors"
                >
        ...
        {/* Terminal Output */}
        <div className="h-28 bg-[#181818] border-t border-[#2a2a2a] p-3 text-xs flex flex-col justify-between shrink-0">
          <div className="flex items-center gap-2 text-[#808080]">
            <Terminal className="w-3.5 h-3.5 text-[#4ec9b0]" />
            <span>Terminal Output</span>
          </div>
          <div className="text-[#6a9955]">
            $ npx ts-node srs_test_creator.ts --input=srs_spec.json --out=e2e/tests/<br />
            [SUCCESS] Auto-generated boundary & validation test cases for Playwright test runner.
          </div>
          <div className="text-[#569cd6] flex items-center justify-between border-t border-[#252526] pt-1 text-[11px]">
